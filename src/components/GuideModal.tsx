...
const GROUPS: { title: string; items: { emoji: string; title: string; desc: string }[] }[] = [
  ...
  },
  {
    title: '오늘의 재미',
    items: [
      { emoji: '🎰', title: '지갑 수비 룰렛', desc: '인증 1번 = 룰렛권 1장(무지출은 2장). 돌리면 젤리 5~150개가 랜덤으로!' },
      { emoji: '🐹', title: '절약 요정 (펫)', desc: '인증하면 자라고, 방치하면 어수선해져요. 마이로그에서 젤리로 꾸며요.' },
  ...
  },
  {
    title: '짠친과 함께',
    items: [
      { emoji: '🔒', title: '짠 서클 (기본 공간)', desc: '3~8명이 서로의 인증을 보는 방이에요. 초대 코드나 랜덤 매칭으로 들어가고, 다 같이 주간 보스를 잡아요.' },
      { emoji: '⚖️', title: '판정과 스탬프', desc: '짠친 인증에 응원이나 스탬프(어림도 없지, 그돈씨…)를 찍어요. 같은 스탬프 2표면 판결 확정!' },
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={e => e.stopPropagation()} style={{ maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px 16px 12px', textAlign: 'left' }}>
          ...
          <p style={{ margin: '4px 0 0', fontSize: '12px', color: 'var(--text-sub)' }}>하루 한 줄 인증하면, 나머지는 전부 따라와요.</p>
        </div>
